      ...
      const realPatients = await Promise.all(
        querySnapshot.docs.map(async (patientDoc) => {
          ...
          return {
            ...
            // Inference text persisted in Firestore.
            meshInference: data.meshInference || null,

            // Precomputed 3D brain mesh (Azure ML batch endpoint -> blob storage).
            // Prefer an explicit meshUrl saved on the patient doc; otherwise derive
            // it from the ADNI subject id. null => viewer shows the upload prompt.
            subjectId: data.subjectId || data.adniSubjectId || null,
            meshUrl:
              data.meshUrl ||
...
setSelectedPatientForDT(prev => ({
  ...
  // === DIGITAL TWIN MESH RESOLUTION (precomputed, no on-demand generation) ===
  // The 3D brain is precomputed by the Azure ML batch endpoint and stored in blob.
  // We resolve a static PLY URL here instead of POSTing scans to /generate_ply
  // (that synchronous call is what was timing out). Order of preference:
  //   1. meshUrl already on the patient (from Firestore, or derived from subjectId)
  //   2. a same-session sessionStorage cache
  //   3. derive from the ADNI subject id
  ...
  useEffect(() => {
    ...
    // No precomputed mesh available: leave meshUrl null. Use the manual
    // "Upload DICOM Folder" control to generate one on demand if needed.
