...
export default function DashboardTopBar({ searchQuery, onSearchChange, pendingRequestsCount = 0 }) {
  ...
  return (
    <nav className={`border-b backdrop-blur-sm sticky top-0 z-50 ${
      isLight ? "bg-[#f9fbfa]/88 border-slate-200 shadow-[0_12px_30px_rgba(15,23,42,0.05)]" : "bg-slate-900/80 border-slate-800"
    }`}>
        ...
        <div className="flex items-center justify-between h-16">
            ...
            <div className="relative w-full">
              ...
              <input
                ...
                className={`w-full rounded-lg pl-10 pr-4 py-2 text-sm focus:outline-none transition-all ${
                  isLight
                    ? "bg-white/95 border border-slate-300 text-slate-950 placeholder-slate-400 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100"
          ...
          <div className="flex items-center space-x-1">
            <button className={`relative p-2 rounded-lg transition-colors ${
              isLight ? "text-slate-500 hover:text-slate-950 hover:bg-white" : "text-slate-400 hover:text-white"
            }`}>
              <Bell size={20} />
